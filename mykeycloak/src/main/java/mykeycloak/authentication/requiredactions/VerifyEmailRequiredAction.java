 ...
 * @version $Revision: 1 $
 */
public class VerifyEmailRequiredAction extends VerifyEmail {

	public static final String ID = "Verify Email (mykeycloak)";
	private static final Logger logger = Logger.getLogger(VerifyEmailRequiredAction.class);

    @Override
    public String getDisplayText() {
        return "Verify Email (mykeycloak)";
    }
    ...
    public void requiredActionChallenge(RequiredActionContext context) {
        ...
        if (! Objects.equals(authSession.getAuthNote(Constants.VERIFY_EMAIL_KEY), email)) {
            ...
            challenge = loginFormsProvider.setAttribute("username", email).createResponse(UserModel.RequiredAction.VERIFY_EMAIL);
        }
    ...
    private Response sendVerifyEmail(KeycloakSession session, LoginFormsProvider forms, UserModel user, AuthenticationSessionModel authSession, EventBuilder event) throws UriBuilderException, IllegalArgumentException {
        ...
        return forms.setAttribute("username", user.getEmail()).createResponse(UserModel.RequiredAction.VERIFY_EMAIL);
    }	
}
